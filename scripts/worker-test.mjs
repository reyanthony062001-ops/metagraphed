...
const head = await handleRequest(new Request("https://metagraph.sh/api/v1/subnets", { method: "HEAD" }), env, {});
...
assert.equal(head.headers.get("x-content-type-options"), "nosniff", "API responses should set nosniff");

const apiOptions = await handleRequest(new Request("https://metagraph.sh/api/v1/subnets", { method: "OPTIONS" }), env, {});
assert.equal(apiOptions.status, 204, "API OPTIONS should return 204");
assert.equal(apiOptions.headers.get("access-control-allow-methods"), "GET, HEAD, OPTIONS");

const rpcOptions = await handleRequest(new Request("https://metagraph.sh/rpc/v1/finney", { method: "OPTIONS" }), env, {});
assert.equal(rpcOptions.status, 204, "RPC OPTIONS should return 204");
assert.equal(rpcOptions.headers.get("access-control-allow-methods"), "POST, OPTIONS");

const apiPost = await handleRequest(new Request("https://metagraph.sh/api/v1/subnets", { method: "POST" }), env, {});
assert.equal(apiPost.status, 405, "POST should not be allowed for artifact API routes");
assert.equal(apiPost.headers.get("allow"), "GET, HEAD, OPTIONS");
assert.equal((await apiPost.json()).error.code, "method_not_allowed");

const unknown = await handleRequest(new Request("https://metagraph.sh/api/v1/does-not-exist"), env, {});
...
const source = await handleRequest(new Request("https://metagraph.sh/api/v1/contracts"), env, {});
...
const disabledRpc = await handleRequest(new Request("https://metagraph.sh/rpc/v1/finney", { method: "POST" }), env, {});
...
try {
  ...
  assert.equal(proxied.status, 200, "safe RPC methods can be proxied when explicitly enabled");
  assert.equal(upstreamCalled, true, "safe RPC proxy should call an eligible upstream");
  assert.ok(proxied.headers.get("x-metagraph-rpc-provider"), "proxied responses should expose provider metadata");
} finally {
